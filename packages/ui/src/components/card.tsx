...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // A filed sheet: hard ink edge, square corners, no ring, no lift.
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "text-[0.95rem] leading-snug font-semibold tracking-tight group-data-[size=sm]/card:text-sm",
        className
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      // min-w-0 lets a wide child (a table) scroll instead of stretching the card.
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        // mt-auto keeps the footer on the floor when a grid stretches the card.
        "mt-auto flex items-center border-t border-foreground/15 bg-muted/60 p-(--card-spacing)",
        className
      )}
      {...props}
    />
